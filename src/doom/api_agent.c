...
api_response_t API_PostEpisode(cJSON *req)
{
    ...
    // Snapshots survive an episode, and survive a change of level.
    //
    // A snapshot is self-describing: its header carries gameskill,
    // gameepisode and gamemap, and restoring one calls G_InitNew with them,
    // so it stands its OWN level back up whatever happens to be loaded now.
    // There is therefore nothing for a new episode to invalidate.
    //
    // Discarding them when the level changed is what stopped a search
    // compounding across a campaign. A run that draws its level from the
    // episode's seed changes level almost every episode, so the archive of
    // places already reached was thrown away almost every episode, and the
    // frontier was rebuilt from the front door each time.
    //
    // A caller that wants them gone asks: {"forget": true}.
    {
        cJSON *forget = cJSON_GetObjectItem(req, "forget");

        if (cJSON_IsTrue(forget))
        {
